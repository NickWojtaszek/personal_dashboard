...
import type { InsuranceInfo, PropertyInfo, VehicleInfo } from '../../types';
...
const InsuranceInfoSection: React.FC<InsuranceInfoSectionProps> = ({ policy, isEditing, onSetEditing, onSave, onCancel, properties, vehicles }) => {
                ...
                <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {vehicles && vehicles.length > 0 && (
                        <div>
                            ...
                            <select
                                value={editedData.vehicleId || ''}
                                onChange={e => setEditedData(prev => ({ ...prev, vehicleId: e.target.value || undefined }))}
                                className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-primary focus:border-brand-primary outline-none transition"
                            >
                                <option value="">None</option>
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
            ...
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                ...
                <DetailItem label="Linked Vehicle" value={policy.vehicleId ? vehicles?.find(v => v.id === policy.vehicleId)?.name || 'Unknown' : undefined} />
            </div>
        </div>
    );
};

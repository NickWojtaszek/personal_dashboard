...
export type RegoDocType = 'renewal' | 'payment';

const CURRENCY_OPTIONS = ['AUD', 'GBP', 'USD', 'EUR', 'PLN'];

interface NewRegistrationModalProps {
    onClose: () => void;
    onCreate: (currency: string, docType: RegoDocType, file: File | null) => void;
}
...
const NewRegistrationModal: React.FC<NewRegistrationModalProps> = ({ onClose, onCreate }) => {
    ...
    const handleSubmit = () => {
        onCreate(currency, docType, file);
    };

    const selectClasses = "w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-brand-primary dark:focus:ring-brand-secondary focus:border-transparent";
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-slate-700" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-slate-200 dark:border-slate-700">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">New Vehicle Registration</h2>
                    <p className="text-sm text-slate-500 dark:text-gray-400 mt-1">Upload a rego document. Details will be extracted automatically.</p>
                </div>
                ...
                <div className="p-6 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        {/* Document Type */}
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1.5">Document Type</label>
                    ...
                    <p className="text-xs text-slate-400 dark:text-gray-500">
                        {docType === 'renewal'
                            ? 'Extracts: vehicle details, fees, dates, CTP info'
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1.5">
                            {docType === 'renewal' ? 'Renewal Notice (PDF)' : 'Payment Confirmation (PDF)'}
                        </label>
                        <div
                ...
                <div className="p-6 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3">
                    ...
                    >
                        {file ? 'Create & Extract' : 'Create Registration'}
                    </button>
                </div>
            </div>
        </div>
